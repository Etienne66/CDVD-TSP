...
parser.add_argument('--loss', default='.1*EPE+1*Fl', type=str, metavar='LOSS',
                    choices=['1*EPE','.1*EPE+1*Fl','.1*EPE+1*Fl','.1*EPE+10*Fl','.01*EPE+1*Fl','1*L1','0.84*MSL+0.16*L1'],
...
OptimizerGroup.add_argument('--max_lr', default=1e-5, type=float, metavar='M',
...
def main():
    ...
    if args.evaluate:
        ...
        args.best_EPE, args.best_Fl = validate(val_loader, model, 0, eval_writer, inverse_transform)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train_loss, train_EPE, train_Fl = train(train_loader, model, optimizer, scheduler, epoch, train_writer_iterations)

        # evaluate on validation set
        with torch.no_grad():
            test_EPE, test_Fl = validate(val_loader, model, epoch, train_writer, inverse_transform)
        ...
        is_best = test_EPE < args.best_EPE
        ...
        if is_best:
            model_state_dict = {'state_dict': model.state_dict(),
                                'args': args}
            torch.save(model_state_dict, args.save_path / args.run_path / (args.arch+'model_best.pt'))
...
def train(train_loader, model, optimizer, scheduler, epoch, train_writer):
    ...
    with tqdm(total=len(train_loader), position=1, bar_format='{desc}', desc='Waiting for first train batch') as tqdm_desc:
        ...
        for i, (inputs, target) in enumerate(tqdm_train):
            ...
            loss_total, loss_EPE, loss_Fl = args.loss_Flow_net(output, target)
            ...
            flow2_Fls.update(loss_Fl/torch.sum(mask).item(), torch.sum(mask).item())
    ...
    return losses.avg, flow2_EPEs.avg, flow2_Fls.avg
...
def validate(val_loader, model, epoch, run_writer, inverse_transform):
    ...
    with tqdm(total=len(val_loader), position=1, bar_format='{desc}', desc='Waiting for first test batch') as tqdm_desc:
        ...
        for i, (inputs, target) in enumerate(tqdm_test):
            ...
            # compute output
            flow2_EPE = realEPE(output, target)
            ...
            if i % args.image_freq == 0 and run_writer is not None and j < 3:
                ...
                if epoch == args.start_epoch:
                    ...
                    if inverse_transform is not None:
                        run_writer.add_image('Input{}/From'.format(frame),
                                             inverse_transform(inputs[:, 0, :, :, :][0,:3]).clamp(0,1).cpu(),
                                             0)
                        run_writer.add_image('Input{}/To'.format(frame),
            ...
            tqdm_desc.set_description('Test: EPE {0}\t Fl {1}'.format(
                                       flow2_EPEs,
                                       flow2_Fls))
    ...
    return flow2_EPEs.avg, flow2_Fls.avg
